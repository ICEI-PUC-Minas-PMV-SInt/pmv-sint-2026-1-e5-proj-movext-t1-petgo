...
export default function Servicos() {
  ...
  const [meusServicos, setMeusServicos] = useState<PasseadorServicoResponseDto[]>([]);
  ...
  // Form State
  const [selectedTipo, setSelectedTipo] = useState<TipoPasseioResponseDto | null>(null);
  ...
  const handleOpenAdd = () => {
    ...
    if (disponiveis.length === 0) {
      Alert.alert("Atenção", "Você já oferece todos os tipos de passeios disponíveis no sistema!");
      return;
    }
  ...
  const handleEditPreco = (servico: PasseadorServicoResponseDto) => {
    ...
    if (tipo) {
        setSelectedTipo(tipo);
        setPreco(servico.precoCustomizado.toString());
        setModalPrecoVisible(true);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await passeadorServicoService.salvarPreco({
        ...
        precoCustomizado: parseFloat(preco.replace(",", ".")),
      });
    ...
    <View className="bg-white p-6 rounded-[32px] border-2 border-blue-50 shadow-sm mb-4">
      <View className="flex-row justify-between items-start mb-4">
        ...
        <Text className="text-[#4876A8] font-black text-2xl tracking-tighter">
            R$ {item.precoBase ? item.precoBase.toFixed(2) : item.precoCustomizado.toFixed(2)}
        </Text>
      </View>
      ...
      <View className="flex-row gap-x-3 border-t border-gray-50 pt-4">
        ...
        >
          ...
          <Text className="font-bold text-xs ml-2 uppercase tracking-widest text-[#4876A8]">Alterar Preço</Text>
        </TouchableOpacity>
    ...
    <View className="flex-1 bg-white">
      ...
      >
        <View className="flex-row items-center">
            ...
            <Text className="text-3xl font-black text-[#4876A8] tracking-tighter">Meus Serviços</Text>
        </View>
        <TouchableOpacity 
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <Text className="text-gray-400 font-semibold mb-6">
              Esta é a lista de serviços que você oferece. Toque no + para adicionar novos tipos.
            </Text>
          }
          ListEmptyComponent={
            <View className="items-center justify-center py-20 bg-gray-50 rounded-[40px] border border-gray-100 border-dashed">
              ...
              <Text className="text-gray-400 font-bold mt-4 text-center px-10">Sua lista está vazia. Adicione um serviço para começar a receber agendamentos!</Text>
            </View>
          }
        />
      )}
        ...
        <View className="flex-1 bg-black/60 justify-end">
          <View className="bg-white rounded-t-[50px] p-8 h-[70%]">
            <View className="flex-row justify-between items-center mb-8">
              <Text className="text-2xl font-black text-[#4876A8] tracking-tighter">Escolha um Serviço</Text>
              <TouchableOpacity onPress={() => setModalAddVisible(false)} className="bg-gray-50 p-3 rounded-2xl">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                {tiposGlobais
                  .filter(g => !meusServicos.some(m => m.tipoPasseioId === g.id))
                  .map((tipo) => (
                    <TouchableOpacity 
                        key={tipo.id}
                    ...
                    >
                        <View>
                            <Text className="text-gray-900 font-black text-lg">{tipo.nome}</Text>
                            <Text className="text-gray-400 font-bold text-xs uppercase tracking-widest">{tipo.duracaoMinutos} min</Text>
                        </View>
      ...
      <Modal visible={modalPrecoVisible} animationType="slide" transparent={true}>
        <View className="flex-1 bg-black/60 justify-end">
          <View className="bg-white rounded-t-[50px] p-8 min-h-[50%]">
            <View className="flex-row justify-between items-center mb-10">
              <View>
                <Text className="text-2xl font-black text-[#4876A8] tracking-tighter">Definir Preço</Text>
                <Text className="text-gray-400 font-bold text-sm uppercase tracking-widest mt-1">{selectedTipo?.nome}</Text>
              </View>
              <TouchableOpacity onPress={() => setModalPrecoVisible(false)} className="bg-gray-50 p-3 rounded-2xl">
                <Feather name="x" size={20} color="#D1D5DB" />
              </TouchableOpacity>
            </View>
            ...
            <View className="gap-y-8">
                <View>
                    <Text className="text-gray-500 text-xs font-bold uppercase tracking-widest ml-1 mb-2">Quanto você quer cobrar?</Text>
                    <View className="bg-gray-50 p-6 rounded-3xl border-2 border-gray-100 flex-row items-center">
                        <Text className="text-2xl font-black text-gray-300 mr-2">R$</Text>
                        <TextInput
                            placeholder="0,00"
                            value={preco}
                ...
                >
                  {saving ? <ActivityIndicator color="white" /> : (
                    <Text className="text-white font-black text-lg uppercase tracking-widest">Confirmar Serviço</Text>
                  )}
                </TouchableOpacity>
            </View>
          </View>
